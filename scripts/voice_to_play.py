...
def voice_to_remind():
    wave_file = wave.open('/robot/remind/开始.wav', 'rb')
...
def voice_to_play():
    wave_file = wave.open('/robot/wav/synthesized_voice.wav', 'rb')
    dev_to_play = pyaudio.PyAudio()
    stream = dev_to_play.open(format=dev_to_play.get_format_from_width(wave_file.getsampwidth()),
                              channels=wave_file.getnchannels(), rate=wave_file.getframerate(), output=True)
    while True:
...
if __name__ == '__main__':
    rospy.init_node('voice_to_play', anonymous=True)
